...
def helper(ctx):
    ...
    cogs = ctx.azami.cogs
    for i in cogs:
        cmd_ = ctx.azami.get_cog(i).get_commands()
        ...
        for x in list(chunks(list(cmd_), 6)):
            ...
            embed.description = ctx.azami.cogs[i].__doc__
            for y in x:
...
async def paginate(ctx, input_):
    ...
    while paging:
        ...
        done, pending = await asyncio.wait([ctx.azami.wait_for('reaction_add', check=check, timeout=120),
                                            ctx.azami.wait_for('reaction_remove', check=check, timeout=120)],
        ...
        for future in pending:
            ...
            if str(reaction.emoji) == r[2]:
                ...
                try:
                    msg = int((await ctx.azami.wait_for('message', check=pager, timeout=60)).content)
                except asyncio.TimeoutError:
...
class Help(commands.Cog):
    ...
    async def help(self, ctx, *, command=None):
        ...
        if command:
            thing = ctx.azami.get_cog(command) or ctx.azami.get_command(command)
            if not thing:
